...
DownloadFormat () {
	...
	if [ "$audioFormat" == "native" ]; then
		...
		XDG_CONFIG_HOME=/config/extended tidal-dl-sg cfg quality_audio LOSSLESS
		deemixQuality=flac
		bitrateError=""
		audioFormatError=""
	fi
...
TidalClientSetup () {
	...
	XDG_CONFIG_HOME=/config/extended tidal-dl-sg cfg extract_flac true 2>&1 | tee -a "/config/logs/$logFileName"
...
DownloadProcess () {
	...
	do	
		...
		if [ "$2" == "TIDAL" ]; then
			...
			XDG_CONFIG_HOME=/config/extended tidal-dl-sg cfg download_base_path "$audioPath/incomplete" 2>&1 | tee -a "/config/logs/$logFileName"
			XDG_CONFIG_HOME=/config/extended tidal-dl-sg cfg quality_audio "$tidalQuality" 2>&1 | tee -a "/config/logs/$logFileName"
			XDG_CONFIG_HOME=/config/extended tidal-dl-sg dl "https://tidal.com/browse/album/$1" 2>&1 | tee -a "/config/logs/$logFileName"
...
CalculateTitleDistance () {
	...
	if ! distance=$(python3 -c 'import sys; from pyxdameraulevenshtein import damerau_levenshtein_distance; print(damerau_levenshtein_distance(sys.argv[1], sys.argv[2]))' "${1,,}" "${2,,}" 2>/dev/null); then
		return 1
	fi
...
ArtistTidalSearch () {
	...
	for tidalArtistAlbumId in $(echo $tidalArtistAlbumsIds); do
		...
		if ! diff=$(CalculateTitleDistance "$lidarrAlbumReleaseTitleClean" "$tidalAlbumTitleClean"); then
